...
flowchart TB
  ...
  Qcomp{"¿Harás operaciones de cómputo grandes
  (punto flotante, DSP, cifrado, control muy rápido)?"}

  Qmem{"¿Tu firmware cabe aprox. en 32 KB de Flash
  y la RAM necesaria es baja (del orden de pocos KB)?"}

  Qconn{"¿Necesitas conectividad avanzada integrada
  (Wi-Fi, BLE o Ethernet)?"}

  Qfoot{"¿Necesitas huella pequeña o empaques sencillos
  para prototipado (DIP o TQFP pequeños)?"}

  Qcost{"¿El costo por unidad es un factor crítico?"}

  Qvcc{"¿Requieres alimentación de 5 V nativa
  o un rango amplio de Vcc?"}

  AVR["Elige un MCU de 8 bits (p. ej., ATmega328P)"]
  M32["Evalúa MCU de 32 bits (Cortex-M o RISC-V 32)"]
  MOD8["Opción: 8 bits + módulo externo de conectividad"]
  UL8["8 bits orientado a ultra bajo consumo (p. ej., MSP430 o AVR tiny)"]
